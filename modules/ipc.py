...
class IPCManger:
    ...
    def send_request(
        self, worker_type: str, model_name: str, uuid: str, payload: Dict[str, Any]
    ) -> None:
        ...
        with open(request_file, "w") as f:
            json.dump({"uuid": uuid, "payload": payload, "timestamp": time.time()}, f)
    ...
    def send_response(
        self, worker_type: str, model_name: str, uuid: str, response: Any
    ) -> None:
        ...
        with open(response_file, "w") as f:
            ...
            try:
                json.dump(
                    {"uuid": uuid, "response": response, "timestamp": time.time()}, f
                )
            except TypeError:
                # For non-JSON serializable objects, store a placeholder and use pickle in a separate file
                json.dump(
                    {"uuid": uuid, "response": "__PICKLED__", "timestamp": time.time()},
                    f,
                )
    ...
    def get_response(
        self, worker_type: str, model_name: str, uuid: str, timeout: float = 60.0
    ) -> Optional[Any]:
            ...
            if os.path.exists(response_file):
                try:
                    ...
                    if response_data.get(
                        "response"
                    ) == "__PICKLED__" and os.path.exists(pickle_file):
