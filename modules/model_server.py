...
class ModelServer:
    def __init__(self):
        ...
        self.workers_config = (
            self.manager.dict()
        )  # Just stores configuration, not actual workers
                ...
                with self._workerLock:
                    ...
                    if model_name not in self.workers_config[type]:
                        ...
                        worker_config: Dict[str, Any] = {
                            ...
                            "type": type,
                            "max_workers": (
                                4 if model_name == "Qwen/Qwen2.5-14B-Instruct" else 1
                            ),
                        }
                        ...
                        if type == "text-generation":
                            p = Process(
                                target=self._run_text_generation_worker,
                                args=(model_name, worker_config),
                            )
                            logger.info(
                            ...
                            p = Process(
                                target=self._run_image_text_to_text_worker,
                                args=(model_name, worker_config),
                            )
                            logger.info(
                            ...
                            p = Process(
                                target=self._run_sent_trans_worker,
                                args=(model_name, worker_config),
                            )
                            logger.info(f"Created sent-trans worker for {model_name}")
                        elif type == "text-generation-bnb":
                            p = Process(
                                target=self._run_text_generation_bnb_worker,
                                args=(model_name, worker_config),
                            )
                            logger.info(
                                f"Created text-generation-bnb worker for {model_name}"
                            )
                        elif type == "clip":
                            p = Process(
                                target=self._run_clip_worker,
                                args=(model_name, worker_config),
                            )
                            logger.info(f"Created CLIP worker for {model_name}")
                        elif type == "genai":
                            p = Process(
                                target=self._run_genai_worker,
                                args=(model_name, worker_config),
                            )
                            logger.info(f"Created genai worker for {model_name}")
                        elif type == "ollama":
                            p = Process(
                                target=self._run_ollama_worker,
                                args=(model_name, worker_config),
                            )
                            logger.info(f"Created ollama worker for {model_name}")
                        elif type == "openai":
                            p = Process(
                                target=self._run_openai_worker,
                                args=(model_name, worker_config),
                            )
    ...
    def _run_text_generation_bnb_worker(self, model_name: str, config: Dict[str, Any]):
        worker = TextGenerationBnbWorker(
            model_name=model_name,
            max_workers=config["max_workers"],
            type=config["type"],
        )
    ...
    def start_flask_server(self):
        ...
        def infer():
            ...
            return (
                jsonify(
                    {
                        "error": "Worker communication not implemented yet",
                        "status": "FAILED",
                    }
                ),
                501,
