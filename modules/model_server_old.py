...
def call_openai_batch(  # only for chat completions
    ...
    if not batch_id:
        response = client.files.create(
            file=("requests.jsonl", jsonl_file), purpose="batch"
        )
        file_id = response.id
        ...
        for result in results:
            if result and result["custom_id"] in input_ids:
                outputs[result["custom_id"]] = {
                    "response": result["response"]["body"]["choices"][-1]["message"][
                        "content"
                    ],
    ...
    for msg in messages:
        ...
        if isinstance(content, str):
            ...
            if use_cache and cache_control:
                message_content[0]["cacheControl"] = {
                    "type": cache_control.get("type", "ephemeral")
                }

            claude_messages.append(
                {"role": msg.get("role", "user"), "content": message_content}
            )
                    ...
                    if "text" in item and "type" not in item:
                        ...
                        if item_type == "text":
                            ...
                            if item_type == "image_url":
                                ...
                                logger.warning(
                                    "Image URLs require conversion to base64 - skipping"
                                )
                            elif "source" in item:
                                source = item.get("source", {})
                                if source.get("type") == "base64":
...
class ModelServer:
    ...
    def save_completed_requests(self, type: str, model_name: str) -> None:
        ...
        try:
            if (
                type not in self.completed_requests
                or model_name not in self.completed_requests[type]
            ):
                return
            ...
            data = {
                uuid: entry
                for uuid, entry in self.completed_requests[type][model_name].items()
            ...
            logger.debug(
                f"Saved {len(data)} completed requests for {type}/{model_name} to {file_path}"
            )
        except Exception as e:
            logger.error(
                f"Failed to save completed requests for {type}/{model_name}: {str(e)}"
            )
            logger.debug(f"Traceback: {traceback.format_exc()}")
            ...
            for file in files:
                try:
                    ...
                    if model_name not in self.completed_requests[type_name]:
                        self.completed_requests[type_name][
                            model_name
                        ] = self.manager.dict()
                    ...
                    logger.info(
                        f"Loaded {len(data)} completed requests for {type_name}/{model_name} from {file_path}"
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to load completed requests from {file}: {str(e)}"
                    )
                    logger.debug(f"Traceback: {traceback.format_exc()}")
        ...
        def infer():
            try:
                ...
                if not payload or not model_name or not type:
                    ...
                    return (
                        jsonify({"error": "Missing payload or model_name or type"}),
                        400,
                    )
    ...
    def check_and_create_worker(self, type: str, model_name: str, timeout: int = 300):
        ...
        if timeout == 0:
            while True:
                with self._workerLock, self._workerLockM:
                    if (
                        type in self.workerProcs
                        and model_name in self.workerProcs[type]
                    ):
                        return
        else:
            ...
            if request:
                ...
                with self._workerLockM:
                    ...
                    if model_name not in self.workerProcs[type]:
                        ...
                        self.workerProcs[type][model_name][
                            "readylockM"
                        ] = self.manager.Lock()
                        self.workerProcs[type][model_name][
                            "new_request_event"
                        ...
                        self.workerProcs[type][model_name][
                            "completed_request_event"
                        ] = self.manager.Event()

                        logger.info(
                            f"Created worker for {type} - {model_name}: {self.workerProcs[type][model_name]}"
                        )
                        ...
                        if type == "text-generation":
                            p = Process(
                                target=self.text_generation_start_processor,
                                kwargs=worker_config,
                            )
                            logger.info(
                                f"Created text-generation worker for {model_name}"
                            )
                        elif type == "image-text-to-text":
                            p = Process(
                                target=self.imageTextToText_start_processor,
                                kwargs=worker_config,
                            )
                            logger.info(
                                f"Created image-text-to-text worker for {model_name}"
                            )
                        elif type == "sent-trans":
                            p = Process(
                                target=self.sent_trans_start_processor,
                                kwargs=worker_config,
                            )
                            logger.info(f"Created sent-trans worker for {model_name}")
                        elif type == "clip":
                            p = Process(
                                target=self.clip_generation_start_processor,
                                kwargs=worker_config,
                            )
                            logger.info(f"Created CLIP worker for {model_name}")
                        elif type == "genai":
                            p = Process(
                                target=self.genai_start_processor, kwargs=worker_config
                            )
                            logger.info(f"Created genai worker for {model_name}")
                        elif type == "ollama":
                            p = Process(
                                target=self.ollama_start_processor, kwargs=worker_config
                            )
                            logger.info(f"Created ollama worker for {model_name}")
                        elif type == "openai":
                            p = Process(
                                target=self.openai_start_processor, kwargs=worker_config
                            )
                            logger.info(f"Created openai worker for {model_name}")
                        elif type == "vllm-text-generation":
                            p = Process(
                                target=self.vllm_text_generation_start_processor,
                                kwargs=worker_config,
                            )
                            logger.info(
                                f"Created vllm-text-generation worker for {model_name}"
                            )
                        elif type == "claude":
                            p = Process(
                                target=self.claude_start_processor, kwargs=worker_config
                            )
                            logger.info(f"Created claude worker for {model_name}")
                        elif type == "openai-embedding":
                            p = Process(
                                target=self.openai_embedding_start_processor,
                                kwargs=worker_config,
                            )
                            logger.info(
                                f"Created openai-embedding worker for {model_name}"
                            )
                        else:
                ...
                for worker_type in worker_types:
                    if self.ready_requests[worker_type]:
                        ...
                        for model_name in model_names:
                            logger.info(
                                f"Total requests: {len(self.ready_requests[worker_type][model_name])} in {model_name} of {worker_type}"
                            )
                            logger.info(
                                f"Completed requests: {len(self.completed_requests[worker_type][model_name])} in {model_name} of {worker_type}"
                            )
                            logger.info(
                                f"Average response time: {(sum(response['response_time'] for response in self.completed_requests[worker_type][model_name].values()) / len(self.completed_requests[worker_type][model_name])) if self.completed_requests[worker_type][model_name] else 0:.2f} seconds in {model_name} of {worker_type}"
                            )
    ...
    def clean_complete_requests(self, only_clean_errors: bool = False):
        ...
        while True:
            try:
                if self.completed_requests:
                    ...
                    for worker_type in worker_types:
                        if self.completed_requests[worker_type]:
                            model_names = list(
                                self.completed_requests[worker_type].keys()
                            )
                            for model_name in model_names:
                                # Check if worker still exists
                                if (
                                    worker_type not in self.workerProcs
                                    or model_name not in self.workerProcs[worker_type]
                                ):
                                    logger.debug(
                                        f"Worker {worker_type}/{model_name} not found, skipping cleanup"
                                    )
                                    continue

                                uuids = list(
                                    self.completed_requests[worker_type][
                                        model_name
                                    ].keys()
                                )
                                for uuid in uuids:
                                    with self._completedLock, self.workerProcs[
                                        worker_type
                                    ][model_name]["completedlockM"]:
                                        entry = self.completed_requests[worker_type][
                                            model_name
                                        ][uuid]
                ...
                logger.error(
                    f"error in cleaning completed requests: {e}", exc_info=True
                )
            time.sleep(3600)
    ...
    def start(self):
        ...
        cleaner_thread = threading.Thread(
            target=self.clean_complete_requests, args=(False,), daemon=True
        )
    ...
    def enqueue_request(
        ...
        """
        logger.debug(
            f"Enqueueing request {uuid} for {type}/{model_name} (retry: {retry_count}/{max_retries}, force: {force})"
        )
        ...
        if uuid in self.completed_requests[type][model_name]:
            if force:
                logger.info(
                    f"Request {uuid} already completed, removing from completed and re-enqueueing (force=True)"
                )
                with self._completedLock, self.workerProcs[type][model_name][
                    "completedlockM"
                ]:
        ...
        with self._readyLock, self.workerProcs[type][model_name]["readylockM"]:
            ...
            logger.info(
                f"Request {uuid} enqueued. Queue size for {type}/{model_name}: {queue_size}"
            )
            self.workerProcs[type][model_name]["new_request_event"].set()

    def dequeue_request(
        self, type: str, model_name: str, count: int = 1
        ...
        with self.workerProcs[type][model_name]["readylockM"]:
            if self.ready_requests[type][model_name]:
                ...
                return [
                    (key, self.ready_requests[type][model_name].pop(key))
                    for key in requests
                ]
        logger.debug(f"Attempting to dequeue {count} requests for {type}/{model_name}")
        return []
            ...
            while True:
                ...
                with self._completedLock, self.workerProcs[type][model_name][
                    ...
                    if uuid in self.completed_requests[type][model_name]:
                        ...
                        logger.info(
                            f"Got response for {uuid} after {response_time:.2f}s"
                        )
                ...
                if elapsed > 30 and int(elapsed) % 30 == 0:  # Log every 30 seconds
                    logger.warning(
                        f"Still waiting for response for {uuid} ({elapsed:.0f}s elapsed)"
                    )
        else:
            while time.time() - start_time < timeout:
                self.workerProcs[type][model_name]["completed_request_event"].wait()
    ...
    def add_to_completed(
        ...
        **kwargs: Any,
        ...
        with self.workerProcs[type][model_name]["completedlockM"]:
            ...
            logger.info(
                f"Request {uuid} completed in {response_time:.2f}s. Total completed for {type}/{model_name}: {completed_count}"
            )
    ...
    def openai_start_processor(self, **kwargs: Any):
        ...
        for api_key in OPENAI_API_KEYS:
            process = Process(
                target=self.openai_Subprocessor, args=(api_key,), kwargs=kwargs
            )
            process.start()
    ...
    def openai_Subprocessor(self, api_key: str, **kwargs: Any):
        ...
        logger.info(
            f"Starting openai worker with config: {kwargs}  of api key: {api_key[:3]}..."
        )
        ...
        while True:
            ...
            logger.debug(
                f"Active openai subworkers: {len(workers)} of api key: {api_key[:3]}..."
            )
            ...
            if len(workers) < ideal_workers:
                logger.info(
                    f"Spawning new openai subworker (total will be: {len(workers) + 1}) of api key: {api_key[:3]}..."
                )
                for _ in range(ideal_workers - len(workers)):
                    p = Process(
                        target=self.openaiWorker,
                        args=(
                            api_key,
                            shared_data,
                        ),
    ...
    def openai_embedding_start_processor(self, **kwargs: Any):
        ...
        for api_key in OPENAI_API_KEYS:
            process = Process(
                target=self.openai_embedding_Subprocessor,
                args=(api_key,),
                kwargs=kwargs,
            )
            process.start()
    ...
    def openai_embedding_Subprocessor(self, api_key: str, **kwargs: Any):
        ...
        model_name: str = kwargs.get("model_name", "")
        logger.info(
            f"Starting openai embedding worker with config: {kwargs} of api key: {api_key[:3]}..."
        )
        max_workers = max(1, int(0.2 * (multiprocessing.cpu_count())))
        workers: List[Process] = []
        shared_data: Dict[str, Any] = {
        ...
        while True:
            workers = [w for w in workers if w.is_alive()]
            logger.debug(
                f"Active openai embedding subworkers: {len(workers)} of api key: {api_key[:3]}..."
            )
            queue_length = len(self.ready_requests[type][model_name])
            ideal_workers = min(max_workers, (queue_length + 9) // 10)
            if len(workers) < ideal_workers:
                logger.info(
                    f"Spawning new openai embedding subworker (total will be: {len(workers) + 1}) of api key: {api_key[:3]}..."
                )
                for _ in range(ideal_workers - len(workers)):
                    p = Process(
                        target=self.openaiEmbeddingWorker,
                        args=(
                            api_key,
                            shared_data,
                        ),
    ...
    def ollama_start_processor(self, **kwargs: Any):
        ...
        def worker(port: int):
            max_workers: int = kwargs.get("max_workers", 10)
            workers: List[Process] = []
            config: Dict[str, Any] = {"port": port, **kwargs}
            while True:
                workers = [w for w in workers if w.is_alive()]
                queue_length = len(self.ready_requests[type][model_name])
                ideal_workers = min(max_workers, (queue_length + 9) // 10)
                ...
                if len(workers) < ideal_workers:
                    for _ in range(ideal_workers - len(workers)):
                        p = Process(
                            target=self.ollamaWorker, kwargs=config, daemon=True
                        )
                        p.start()
                        workers.append(p)
                self.workerProcs[type][model_name]["new_request_event"].wait()
    ...
    def claude_start_processor(self, **kwargs: Any):
        ...
        while True:
            ...
            if len(workers) < ideal_workers:
                logger.info(
                    f"Spawning new claude worker (total will be: {len(workers) + 1})"
                )
                for _ in range(ideal_workers - len(workers)):
                    worker_config = {**kwargs, "shared_data": shared_data}
                    p = Process(
                        target=self.claudeWorker, kwargs=worker_config, daemon=True
                    )
                    p.start()
                    workers.append(p)
            self.workerProcs[type][model_name]["new_request_event"].wait()
    ...
    def text_generation_start_processor(self, **kwargs: Any):
        ...
        while True:
            ...
            if len(workers) < ideal_workers:
                logger.info(
                    f"Spawning new text generation worker (total will be: {len(workers) + 1})"
                )
                for _ in range(ideal_workers - len(workers)):
                    p = Process(
                        target=self.textGenerationWorker, kwargs=kwargs, daemon=True
                    )
                    p.start()
                    workers.append(p)
            self.workerProcs[type][model_name]["new_request_event"].wait()
    ...
    def vllm_text_generation_start_processor(self, **kwargs: Any):
        ...
        while True:
            ...
            if len(workers) < ideal_workers:
                logger.info(
                    f"Spawning new vllm text generation worker (total will be: {len(workers) + 1})"
                )
                for _ in range(ideal_workers - len(workers)):
    ...
    def sent_trans_start_processor(self, **kwargs: Any):
        ...
        while True:
            ...
            if len(workers) < ideal_workers:
                logger.info(
                    f"Spawning new sent trans worker (total will be: {len(workers) + 1})"
                )
                for _ in range(ideal_workers - len(workers)):
    ...
    def clip_generation_start_processor(self, **kwargs: Any):
        ...
        while True:
            ...
            if len(workers) < ideal_workers:
                logger.info(
                    f"Spawning new clip worker (total will be: {len(workers) + 1})"
                )
                for _ in range(ideal_workers - len(workers)):
    ...
    def imageTextToText_start_processor(self, **kwargs: Any):
        ...
        while True:
            ...
            if len(workers) < ideal_workers:
                logger.info(
                    f"Spawning new image text to text worker (total will be: {len(workers) + 1})"
                )
                for _ in range(ideal_workers - len(workers)):
                    p = Process(
                        target=self.imageTextToTextWorker, kwargs=kwargs, daemon=True
                    )
                    p.start()
                    workers.append(p)
            self.workerProcs[type][model_name]["new_request_event"].wait()
        ...
        while True:
            ...
            if uuid and entry:
                ...
                try:
                    ...
                    self.add_to_completed(
                        type, model_name, uuid, entry, response=response
                    )
                except Exception as e:
                ...
                if (
                    time.time() - last_active > 300
                    and not self.ready_requests[type][model_name]
                ):
                    break

    def openaiEmbeddingWorker(
        self, api_key: str, shared_data: Dict[str, Any], **kwargs: Any
    ):
        type: str = kwargs.get("type", "")
        model_name: str = kwargs.get("model_name", "")
        if not api_key or not shared_data:
        ...
        while True:
            ...
            if selected_batch:
                ...
                try:
                    ...
                    config = {
                        k: v
                        for k, v in selected_batch[0][1]["payload"].items()
                        if k != "input"
                    }
                    payload: Dict[str, Any] = {**config, "input": input_texts}
                    ...
                    for uuid, entry, count in response_counts:
                        ...
                        final_response: List[float] | List[List[float]] = (
                            generated_texts
                            if count > 1 or isinstance(entry["payload"]["input"], list)
                            else generated_texts[0]
                        )

                        self.add_to_completed(
                            type, model_name, uuid, entry, response=final_response
                        )
                except Exception as e:
    ...
    def claudeWorker(self, **kwargs: Any):
        ...
        if not has_shared_data:
            logger.warning(
                f"Claude worker started without shared_data, using local throttling state"
            )
        ...
        logger.info(
            f"Starting claude worker for {type}/{model_name} (shared_data: {has_shared_data})"
        )
        logger.debug(f"Worker config: {kwargs}")
        last_active = time.time()
            ...
            if has_shared_data:
                with shared_data["throttle_lock"]:
                    if now - shared_data["last_throttle_time"].value > 60:
                        if shared_data["consecutive_throttles"].value > 0:
                            logger.info(
                                f"Resetting shared throttle backoff after 60s of no throttling"
                            )
                ...
                if now - local_last_throttle > 60:
                    if local_throttles > 0:
                        logger.info(
                            f"Resetting local throttle backoff after 60s of no throttling"
                        )
                    local_throttles = 0
            ...
            if requests:
                ...
                if has_shared_data:
                    with shared_data["throttle_lock"]:
                        ...
                        if current_throttles > 0:
                            delay = (
                                2 ** min(current_throttles - 1, 5)
                            ) + random.uniform(0, 1)
                            logger.info(
                                f"Pre-emptive backoff due to shared throttling (count: {current_throttles}), waiting {delay:.2f}s before request"
                            )
                else:
                    current_throttles = local_throttles
                    if current_throttles > 0:
                        delay = (2 ** min(current_throttles - 1, 5)) + random.uniform(
                            0, 1
                        )
                        logger.info(
                            f"Pre-emptive backoff due to local throttling (count: {current_throttles}), waiting {delay:.2f}s before request"
                        )
                ...
                try:
                    response = call_claude(
                        entry["payload"], model_id=model_name, region_name=region_name
                    )
                    self.add_to_completed(
                        type, model_name, uuid, entry, response=response
                    )
                    ...
                    if has_shared_data:
                        with shared_data["throttle_lock"]:
                            if shared_data["consecutive_throttles"].value > 0:
                                logger.info(
                                    f"Request succeeded, resetting shared throttle counter from {shared_data['consecutive_throttles'].value} to 0"
                                )
                            shared_data["consecutive_throttles"].value = 0
                    else:
                        if local_throttles > 0:
                            logger.info(
                                f"Request succeeded, resetting local throttle counter from {local_throttles} to 0"
                            )
                        local_throttles = 0
                    ...
                    if is_throttling:
                        ...
                        logger.warning(
                            f"Throttling detected (consecutive: {current_throttles}), waiting {delay:.2f}s before retry"
                        )
                        time.sleep(delay)
                    else:
                        time.sleep(60)
        ...
        while True:
                    ...
                    try:
                        with self._gpuLock, self._gpuLockM:
                            available_gpus = get_optimal_gpu_set(
                                model_name, dtype="fp16"
                            )
                            logger.debug(
                                f"Available GPUs for {model_name}: {available_gpus}"
                            )
                            if available_gpus:
                                logger.info(
                                    f"Loading model {model_name} on GPU: {available_gpus[0]}"
                                )
                                ...
                                logger.warning(
                                    f"No available GPUs for {model_name}, waiting..."
                                )
            ...
            if pipe and self.ready_requests[type][model_name]:
                ...
                for uuid, entry in raw_requests:
                    ...
                    inputs: List[Dict[str, str]] | List[List[Dict[str, str]]] = payload[
                        "inputs"
                    ]
                    ...
                    try:
                        ...
                        for uuid, entry, count in response_counts:
                            ...
                            final_response: str | List[str] = (
                                generated_texts
                                if count > 1
                                or isinstance(entry["payload"]["inputs"][0], list)
                                else generated_texts[0]
                            )
    ...
    def vllmTextGenerationWorker(self, **kwargs: Any):
        ...
        while True:
            ...
            if pipe and self.ready_requests[type][model_name]:
                ...
                for uuid, entry in raw_requests:
                    payload = entry["payload"]
                    sentences: List[str | Dict[str, Any]] | str | Dict[str, Any] = (
                        payload["inputs"]
                    )
                ...
                if selected_batch:
                    ...
                    try:
                        ...
                        for uuid, entry in selected_batch:
                            inputs: (
                                List[str | Dict[str, Any]] | str | Dict[str, Any]
                            ) = entry["payload"]["inputs"]
                                ...
                                if isinstance(input_text, dict):
                                    if "multi_modal_data" in input_text:
                                        if "image" in input_text["multi_modal_data"]:
                                            logger.info(
                                                f'{input_text["multi_modal_data"]["image"]}'
                                            )
                                            input_text["multi_modal_data"]["image"] = [
                                                to_pil_image(
                                                    input_text["multi_modal_data"][
                                                        "image"
                                                    ]
                                                )
                                            ]
                        ...
                        for uuid, entry, count in response_counts:
                            ...
                            final_response: str | List[str] = (
                                generated_texts
                                if count > 1
                                or isinstance(entry["payload"]["inputs"], list)
                                else generated_texts[0]
                            )
        ...
        while True:
            ...
            if pipe and self.ready_requests[type][model_name]:
                ...
                for uuid, entry in raw_requests:
                    ...
                    # Exclude `messages` from the payload key for grouping
                    key = frozenset(
                        (k, v) for k, v in payload.items() if k != "sentences"
                    )
                    grouped[key].append((uuid, entry))
                else:
                    # If not a pre-batched request, pick one uniform group (up to 8)
                ...
                if selected_batch:
                    ...
                    try:
                        ...
                        for uuid, entry, count in response_counts:
                            outputs = responses[index : index + count]
                            generated_texts: List[List[float]] = [
                                o.tolist() for o in outputs
                            ]
                            index += count
                            ...
                            final_response: List[float] | List[List[float]] = (
                                generated_texts
                                if count > 1
                                or isinstance(entry["payload"]["sentences"], list)
                                else generated_texts[0]
                            )
