...
class Worker:
    ...
    def printStats(self):
        while True:
            time.sleep(60)
            logger.info(
                f"Total requests: {len(self.ready_requests)} in {self.model_name} of {self.type}"
            )
            logger.info(
                f"Completed requests: {len(self.completed_requests)} in {self.model_name} of {self.type}"
            )
            logger.info(
                f"Time elapsed: {time.time() - self.start_time:.2f} seconds in {self.model_name} of {self.type}"
            )
            logger.info(
                f"Average response time: {(sum(response['response_time'] for response in self.completed_requests.values()) / len(self.completed_requests)) if self.completed_requests else 0:.2f} seconds in {self.model_name} of {self.type}"
            )
    ...
    def enqueue_request(self, uuid: str, payload: Dict[str, Any]) -> None:
        ...
        with self._requestLock:
            self.ready_requests[uuid] = {"payload": payload, "timestamp": time.time()}
